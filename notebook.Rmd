...
y_pred_prior = array(rep(1, 365*5*4), dim=c(N, K, D-1))

for (j in 1:11) { # number of age groups
  mu_group_alpha = rnorm(N, 0, 20000) # based on previous studies
  mu_group_beta = rnorm(N, 0, 5) # based on previous studies
    
  for (q in 1:N) {
    while (TRUE) {
      tau_alpha = rcauchy(1, 0, 5000)
      tau_beta = rcauchy(1, 0, 5)
    ...
    # print(paste0("a ", alpha))
    # print(paste0("b ", mu_group_beta[q], "  ", tau_beta))
    for (i in 2:10) { # number of years
      mu = alpha + beta * hierarchical_data[j, i-1]
...
```{R}
hist(y_pred_prior[y_pred_prior < 100000], col = 'skyblue3')
```


No values exceeded 1000000, which is 1/5 population of Finland. Thus, the prior makes sense
...
fit_hierarchical = sampling(stan_model_ar_hierarchical,data = stan_data, seed = 12452, chains = 7, iter = 6000, control=list(max_treedepth=15))
print(fit_hierarchical)
```
