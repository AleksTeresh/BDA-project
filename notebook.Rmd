...
j = 1
for (q in 1:N) {
  # print(tau_alpha[q] > 0 && tau_beta[q] > 0)
  if (tau_alpha[q] > 0 && tau_beta[q] > 0) {
    alpha = rnorm(1, mu_group_alpha[q], tau_alpha[q])
    beta = rnorm(1, mu_group_beta[q], tau_beta[q])
...
for (j in 1:11) { # number of age groups
  mu_group_alpha = rnorm(N, 0, 10000)
  mu_group_beta = rnorm(N, 0, 2)
    
  for (q in 1:N) {
    while (TRUE) {
      tau_alpha = rcauchy(1, 0, 500)
      tau_beta = rcauchy(1, 0, 1)
...
model {
  mu_group_alpha ~ normal(0, 10000); // weakly informative prior
  mu_group_beta ~ normal(0, 2); // weakly informative prior
  tau_alpha ~ cauchy(0, 500); // weakly informative prior
  tau_beta ~ cauchy(0, 1); // weakly informative prior
