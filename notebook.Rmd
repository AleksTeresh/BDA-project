...
From teh results above, we ca nsee that 
...
```{r}
loglik_exact_hierarchical <- get_lfo_cv_results(10000, 2, 200, 2)
...
plots <- lapply(
  ...
  function(group_idx) {
    ...
    ppc_ribbon(c(full_grouped_data[group_idx, (2:10)]), y_rep[, indexes]) 
  }
)
...
plots <- lapply(
  ...
  function(group_idx) {
    indexes <- seq(group_idx, 99, 11)
    ppc_intervals(c(full_grouped_data[group_idx, (2:10)]), y_rep[, indexes])
  }
)
...
```{r}
loglik_exact_hierarchical <- get_lfo_cv_results(10000, 25, 2000, 25)
exact_elpds_1sap <- apply(loglik_exact_hierarchical, 2, log_mean_exp)
...
fit_hierarchical_narrow_prior = sampling(
  stan_model_ar_hierarchical,
  data = stan_data,
  seed = 12452,
  refresh=0,
...
```{r}
loglik_exact_hierarchical <- get_lfo_cv_results(2000, 0.5, 10, 0.1)
exact_elpds_1sap <- apply(loglik_exact_hierarchical, 2, log_mean_exp)
...
```{r}
check_prior(full_grouped_data, 0, 10000, 0, 2)
```

```{r}
check_prior(full_grouped_data, 0, 10000, 1, 1)
```

```{r}
check_prior(full_grouped_data, 0, 10000, 0, 25)
```

```{r}
check_prior(full_grouped_data, 0, 30000, 0, 2)
```
...
```{r}
loo_separate <- get_loo_diagnostics(log_lik_separate)
print(loo_separate)
plot(loo_separate)
```
...
```{r}
get_lfo_cv_results <- function(alpha_prior_sd, beta_prior_sd) {
  N = 10
  L = 4
  K = 11
  ...
  for (i in seq(N, L+1, -1)) {
    ...
    fit_i = sampling(stan_model_ar_separate, data = stan_data_i, seed = 12452, refresh=0, iter = 5000, control=list(max_treedepth=15))
    last_K_values <- seq(((i-2) * K + 1), ((i - 1) * K))
...
```{r}
loglik_exact_separate <- get_lfo_cv_results(10000, 2)
...
```
```{r}
loo_hierarchical$estimates[1:2]
```
...
comparison_results = matrix(
  c(
    ...
    loo_separate$estimates[1:2],
    exact_elpd_separate
  ),
  ncol = 2,
  dimnames = list(c("elpd_loo", "p_loo", "elpd_lfo"), c("hierarchical", "separate"))
)
comparison_results
```
...
fit_separate_wide_prior = sampling(
  stan_model_ar_separate,
  data = stan_data,
  seed = 12452,
  refresh=0,
...
```{r}
y_rep <- as.matrix(fit_separate_wide_prior, pars = "y_rep")
plots <- lapply(
  seq(1, 11),
  function(group_idx) {
    indexes <- seq(group_idx, 99, 11)
...
```{r}
loglik_exact_separate <- get_lfo_cv_results(10000, 25)
exact_elpds_1sap <- apply(loglik_exact_separate, 2, log_mean_exp)
...
fit_separate_narrow_prior = sampling(
  stan_model_ar_separate,
  data = stan_data,
  seed = 12452,
  refresh=0,
...
```{r}
loglik_exact_separate <- get_lfo_cv_results(2000, 0.5)
exact_elpds_1sap <- apply(loglik_exact_separate, 2, log_mean_exp)
