...
*Note: * most of the code chunks in the notebook are a single call to a function that does all the work under the hood. Implementations of all the functions used can be found at the end of the notebook in the Appendix section. 


# Introduction

Finland is often praised for its achievements in overall safety and very low crime rates, yet the [report](https://yle.fi/uutiset/osasto/news/article10531775.ece) by Finnish National Institute for Health and Welfare indicated that 130,000 people were victims of domestic violence in 2017. Moreover, according to this [source](https://yle.fi/uutiset/osasto/news/finland_is_eus_second_most_violent_country_for_women/7120601), Finland was EU’s second most violent country for women in 2014. Taken by surprise by this information, we decided to look more into the issue. Eventually, it seemed like a great idea to us to use the dataset described below as a basis for the project to understand the issue ourselves and to raise awareness among other students. In this notebook, we study the number of domestic violence cases per year grouped by victims' age categories.
...
The dataset was obtained from Tilastokeskus [Statistic Finland](http://pxnet2.stat.fi/PXWeb/pxweb/en/StatFin/StatFin__oik__rpk__uhri/statfin_rpk_pxt_11ch.px/) and contains data about the number cases of domestic violence and intimate partner violence reported as offence.
...
More information about the dataset could by found following [this link](https://www.stat.fi/til/rpk/2018/15/rpk_2018_15_2019-06-06_tie_001_en.html) or [this link](http://pxnet2.stat.fi/PXWeb/pxweb/en/StatFin/StatFin__oik__rpk__uhri/statfin_rpk_pxt_11ch.px/)
    ...
    data_for_age_group <- filter(data, Victim.s.age == ageGroups[i])
     ggplot(aes(Year, Number.of.victims), data = data_for_age_group) +
      geom_point(size = 1)  + geom_line() +
       labs(
         title=paste0("Reported cases. Victim's age: ", ageGroups[i]),
         x ="Year",
         y = "Number of victims"
       )
  }
