...
fit_hierarchical = sampling(stan_model_ar_hierarchical, data = stan_data, iter = 15000, control=list(max_treedepth=15))
print(fit_hierarchical)
```
...
```{r}
samples_poisson <- rstan::extract(fit_hierarchical, permuted = T)
loglik_exact <- matrix(nrow = length(samples_poisson), ncol = N)
```

```{r}

for (i in N:max(L + 1, 2)) {
  fit_i <- update(fit_hierarchical, newdata = df[-(i:N), ], recompile = FALSE)
